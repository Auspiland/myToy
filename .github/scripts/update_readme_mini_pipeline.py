...
# -*- coding: utf-8 -*-
"""
Mini-Pipeline 프로젝트 README 자동 업데이트 스크립트

Git diff를 분석하여 LLM을 통해 Mini-Pipeline/README.md의 자동 업데이트 섹션을 갱신합니다.
"""
...
def llm_summarize(openai_key: str, model: str, prompt: str) -> str:
    ...
    resp = client.responses.create(
        model=model,
        input=[
            {"role": "system", "content": "You are a technical documentation expert specializing in data pipeline and streaming systems."},
...
TEMPLATE = """Mini-Pipeline 프로젝트 README 자동 업데이트

## 프로젝트 정보
- YouTube URL → 문장전처리 및 Kafka, OpenSearch upload E2E 파이프라인
- 기술: Docker, Kafka, Spark, OpenSearch, FastAPI, Nginx, Kiwipiepy
...
## 업데이트 지침
- 컨테이너/네트워크 변경 → 아키텍처 섹션 업데이트
- Spark/Kafka 변경 → 파이프라인 섹션 업데이트
- API 변경 → 웹 서비스 섹션 업데이트
- Docker 설정 변경 → 배포 섹션 업데이트
- 전처리 로직 변경 → 데이터 처리 섹션 업데이트
- 중요 변경 → 버전 히스토리 추가

## 출력 요구사항
- 한국어로 작성
- 기술적으로 정확하게: 컨테이너명, 토픽명, 포트번호 명시
- 코드 블록 사용: Docker 명령어 및 설정
