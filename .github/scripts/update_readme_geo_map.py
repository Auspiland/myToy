...
# -*- coding: utf-8 -*-
"""
geo_map 프로젝트 README 자동 업데이트 스크립트

Git diff를 분석하여 LLM을 통해 geo_map/README.md의 자동 업데이트 섹션을 갱신합니다.
"""
...
def llm_summarize(openai_key: str, model: str, prompt: str) -> str:
    ...
    resp = client.responses.create(
        model=model,
        input=[
            {"role": "system", "content": "You are a technical documentation expert specializing in geospatial data processing projects."},
...
TEMPLATE = """geo_map 프로젝트 README 자동 업데이트

## 프로젝트 정보
- 좌표/사각형 영역을 한반도 행정구역으로 변환하는 프로젝트
- 기술: Python, Shapely, GeoPandas, GeoJSON, Shapefile
...
## 업데이트 지침
- 파일 추가/삭제 → 디렉토리 구조 업데이트
- API 변경 → 사용법 및 예시 업데이트
- 성능 개선 → 성능 수치 업데이트
- 스크립트 추가 → 유틸리티 섹션 업데이트
- 중요 변경 → 버전 히스토리 추가

## 출력 요구사항
- 한국어로 작성
- 기술적으로 정확하게: 함수명, 파일경로, 성능수치 명시
- 코드 블록 사용: 예시 코드와 디렉토리 트리
