name: Auto Update geo_map README
  ...
  update-readme:
    ...
    steps:
      ...
      - name: Run updater
        ...
        run: |
          python .github/scripts/update_readme_geo_map.py \
      ...
      - name: Commit & Push if changed
        run: |
          if [[ -n "$(git status --porcelain)" ]]; then
            ...
            git add -A
            git commit -m "chore(geo_map): auto-update README via LLM [skip ci]"
            git push
          else
