...
class Rect2Region():
    ...
    def _build_caches(self):
        ...
        # 3. Spatial index 구축 - geometry와 feature_data를 함께 저장
        self.geometry_list = []  # index로 접근 가능한 리스트
        self.feature_data_list = []  # geometry와 같은 index로 매핑

        for idx, feature_data in enumerate(self.features):
            g, pg, nm, all_names, center_coords = _unpack_feature(feature_data)

            self.geometry_list.append(g)
            self.feature_data_list.append(feature_data)
        ...
        # STRtree 구축 (공간 인덱스)
        self.spatial_index = STRtree(self.geometry_list)

    def get_region_representative_name(self, region_name, coverage_ratio, rect_center_lon, rect_center_lat, island_info=None, intersection=None):
        """
        광역 단위 지역의 포괄적 표현 생성 (캐시 활용 + 섬 감지).
    ...
    def query_regions_in_rect_fast(self, rect_coords, only_representive_text=False) -> dict:
        """
        사각형 영역에 포함되는 행정구역들을 반환 (고속 버전 v2 - 섬 감지 추가).
        ...
        if only_representive_text:
            ...
            for prov_feature in self.provincial_features:
                ...
                # provincial_regions 업데이트 (기존 coverage와 비교)
            ...
            # Representative 생성
            representative = [
                self.get_region_representative_name(
                    region_name, region_data["coverage"],
    ...
    def convert(self, rect_coords, only_representive_text=False):
        """고속 변환 함수 v2 (spatial index + 섬 감지)"""
...
if __name__ == "__main__":
    ...
    result4 = converter.convert(rect_coords_jeju, only_representive_text=True)
    converter.show(result4)
